...
class KmeansGA(object):
    ...
    def initialize(self, SamplingMethod = "LHC", InitialPop = None):
        ...
        self.Pop[0] = {}
        self.PopRes[0] = {}         # Result PopRes[gen][sp][Loss/Dmin/Feasibility/SelfD]: 1D array with length of population size.
        self.SPCentroid[0] = {}
        self.KPopRes[0] = {}
        
        # Populate pop for each SP. [KmeansGA: Only SP0, where SPList contain only SP0]
    ...
    def nextGen(self):
        ...
        Feasibility_sp0 = self.PopRes[CurrentGen]["SP0"]["Feasibility"]
        Loss_sp0 = self.PopRes[CurrentGen]["SP0"]["Loss"]
        ...
        Pop_sp0 = self.Pop[CurrentGen]["SP0"]
        ...
        for k in range(SelectedK):
            ...
            for i in range(SubPopSize):
                ...
                KParentIndex[k][i] = pair[ np.argmin( [Loss_sp0[pair[0]],  
                                                       Loss_sp0[pair[1]]] ) ]
            ...
            for p in range(int(SubPopSize/2)):     # PopSize must be even.
                parent1 = Pop_sp0[ int(KParentIndex[k][2*p]) ]     
                parent2 = Pop_sp0[ int(KParentIndex[k][2*p+1]) ]   
            ...
            KChildren[k][0] = Pop_sp0[int(KElliteIndex[k])]
            # Store ellite of each cluster.
            self.KPopRes[CurrentGen]["Ellites"][k] = Pop_sp0[int(KElliteIndex[k])]
        
        # Fill KChildren into new gen of pop.
        self.Pop[CurrentGen+1] = {}
        self.Pop[CurrentGen+1]["SP0"] = np.zeros((PopSize, NumPar))
        for p in range(PopSize):
            self.Pop[CurrentGen+1]["SP0"][p] = KChildren[int(p%SelectedK)][int(p/SelectedK)]
        #---------------------------------------------------------------------------------------
    ...
    def run(self, InitialPop = None):
        ...
        logger.info("Solutions:\n{}".format(self.KPopRes[self.CurrentGen-1]["Ellites"]))
