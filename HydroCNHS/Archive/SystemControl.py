...
def updateConfig(modifiedConfig):
    """Given the dictionary of modified setting, this funciton will over write
    Config.yaml.
...
def writeModel(modelDict, modelname, org_model=None):
    ...
    if org_model is not None:   # Contain comments in the original model file.
        ...
        with open(modelname, 'w') as file:
            SavedModel = yaml.safe_dump(
                modelDict, file, sort_keys=False, default_flow_style=None
                )
...
def writeModelToDF(modelDict, keyOption=["Pars"], prefix=""):
    ...
    Args:
        ...
        KeyOption (list, optional): Output items: Pars, Inputs, Attributions.
            Defaults to ["Pars"].
    ...
    def mergeDicts(DictList):
        ...
        if Len == 0:
            return {}
        ResDict = deepcopy(DictList[0])  # Avoid to modify the original dict. 
        if Len > 1:
        ...
        if s == "LSM" and keyOption != ["Attributions"]:
            ...
            for ro in modelDict[s]:
                if ro != "Model":
                    for o in modelDict[s][ro]:
                        DictList = [modelDict[s][ro][o].get(i) \
                                    for i in keyOption]
                        MergedDict = mergeDicts(DictList)
            ...
            for agtype in AgTypes:
                for ag in modelDict[s][agtype]:
                    DictList = [modelDict[s][agtype][ag].get(i) \
                                for i in keyOption]
                    MergedDict = mergeDicts(DictList)
...
def Dict2String(dictionary, indentor="  "):
    ...
    """
    def Dict2StringList(dictionary, indentor="  ", count=0, string=[]):
        ...
        return string
    return "\n".join(Dict2StringList(dictionary, indentor))
...
def checkWS(model):
    ...
    #--- Check keys
    IdealKeys = ["NumSubbasins", "NumGauges", "NumAgents", "Outlets",
                 "GaugedOutlets"]
    if IdealKeys not in WS.keys():
        logger.error(
            "Missing items in WaterSystem setting. {}".format(IdealKeys))
        Pass = False
...
def checkLSM(model):
    ...
    if model not in LSMOptions:
        logger.error(
            "Invlid LSM model {}. Acceptable options: {}".format(model,
                                                                 LSMOptions)
            )
        Pass = False
    return Pass
def checkAgentInRouting(model):
    ...
    for ISagType in InstreamAgentTypes:
        for ag in model["ABM"][ISagType]:
            ...
            InflowOutlets = [outlet for outlet in Links \
    ...
    # Model["SystemParsedData"]["InStreamAgentInflows"] = InStreamAgentInflows
                    ...
                    if ro != vro:
                        if any( start in others for others in Routing[ro] ):
                            ...
                            if ro in Routing[vro]:  
                                ...
                                logging.error(
                                    """[Check model failed] {} sub-basin outlet
                                    shouldn't be in {}'s (routing outlet) 
                                    catchment outlets due to the seperation of 
                                    in-stream control agents.""".format(
                                        start, vro, ro
                                        )
                                    )
                                Pass = False
...
def parseSimulationSeqence(model):
    ...
    ## In-stream agents here means those agents will redefine the streamflow
    # completely.
    ## which RiverDivAgentTypes, in our definition, only modify the streamflow.
    
    # Contain only "ResDamAgentTypes" and "DamDivAgentTypes".
    InStreamAgents = []
    if model.get("ABM") is not None:
        ABM = model["ABM"]
        for AgTypes in ["ResDamAgentTypes", "DamDivAgentTypes",
                        "RiverDivAgentTypes"]:    
            Agents = []
            for agType in model["ABM"]["Inputs"][AgTypes]:
                for end in ABM[agType]:
                    ...
                    if AgTypes != "RiverDivAgentTypes":
                        InStreamAgents.append(end)
                    Agents.append(end)
            model["SystemParsedData"][AgTypes[:-5] + "s"] = Agents
    # Always have InStreamAgents no matter whether ABM section exists.
    ...
    if model.get("ABM") is not None:
        ...
        for agType in InstreamAgentTypes:
            for end in ABM[agType]:
                ...
                InflowNodes = [node for node in Links \
    ...
    def formSimSeq(Node, BackTrackingDict, RoutingOutlets):
        """A recursive function, which keep tracking back upstream nodes until 
        reach the most upstream one. We design this in a clear way. To 
        understand to logic behind, please run step-by-step using the test 
        example at the bottom of the code.
        """
        SimSeq = []
        def trackBack(node, SimSeq, BackTrackingDict, TempDict={},
                      AddNode=True):
            if AddNode:
                SimSeq = [node] + SimSeq
            if BackTrackingDict.get(node) is not None:
                routingOutlets = [o for o in BackTrackingDict[node] \
                                  if o in RoutingOutlets] 
                if len(routingOutlets) >= 1:
                    # g > 1 or len(g) < len(all) => update TempDict
                    if (len(routingOutlets) > 1 
                        or len(routingOutlets) < len(BackTrackingDict[node])): 
                        # Get rank of each g
                        rank = []
                        ...
                        TempDict.pop(gmax, None)
                        for g in routingOutlets:
                            TempDict[g] = node
                        # Call trackBack with gmax and TempDict (recursive)
                        SimSeq, TempDict, BackTrackingDict = trackBack(
                            gmax, SimSeq, BackTrackingDict, TempDict
                            )
                        
                    elif (len(routingOutlets) == 1
                          and len(BackTrackingDict[node]) == 1):
                        SimSeq, TempDict, BackTrackingDict = trackBack(
                            routingOutlets[0], SimSeq, BackTrackingDict,
                            TempDict
                            )
                        TempDict.pop(routingOutlets[0], None)
                        # Search TempDict and jump backward to add other
                        # tributary.
                        # reverse TempDict
                        rTempDict = {}
                        for g in TempDict:
                        ...
                        if rTempDict != {}:
                            ...
                            ToNode = SimSeq[min([SimSeq.index(i) \
                                            for i in rTempDict])]
                            SimSeq, TempDict, BackTrackingDict = trackBack(
                                ToNode, SimSeq, BackTrackingDict, {}, False
                                )
                else:
                    for up in BackTrackingDict[node]:
                        SimSeq, TempDict, BackTrackingDict = trackBack(
                            up, SimSeq, BackTrackingDict, TempDict
                            )    
            elif TempDict != {}:
        ...
        SimSeq, TempDict, BackTrackingDict = trackBack(
            Node, SimSeq, BackTrackingDict
            )
        return SimSeq
    
    
    # LastNode = End nodes - start node. Last node is the only one that does
    # not exist in start nodes.
    if BackTrackingDict == {}:      # If there is only a single outlet!
        SimSeq = RoutingOutlets
    else:
        # end nodes - start nodes = last node. 
        LastNode = list(
            set(BackTrackingDict.keys()) - set([i[0] for i in Edges])
            )[0]   
        SimSeq = formSimSeq(LastNode, deepcopy(BackTrackingDict),
                            RoutingOutlets)
    model["SystemParsedData"]["SimSeq"] = SimSeq
    # Sort RoutingOutlets based on SimSeq
    model["SystemParsedData"]["RoutingOutlets"] = [ro for ro in SimSeq \
                                                   if ro in RoutingOutlets] 
    #--------------------------------------------------------------------------
    ...
    if model.get("ABM") is not None:
        ...
        for agType in ResDamAgentTypes:
            ...
            for ag in AgList:
                ...
                Piority["AgSimPlus"][ag]["ResDamAgents"].append(0)
        ...
        for agType in DamDivAgentTypes:
            ...
            for ag in AgList:
                ...
                # In-stream agent always has piority 0.
                Piority["AgSimPlus"][ag]["DamDivAgents"].append(0)
            ...
            for ag in AgList:
                ...
                for p in Plus:
                    ...
                    Piority["AgSimPlus"][ro]["RiverDivAgents"].append(
                        model["ABM"][agType][member]["Inputs"]["Piority"]
                        )
                for m in Minus:
        ...
        for agType in InsituDivAgentTypes:
            # InsituDivAgentTypes is a simple diversion agent type, which only
            ...
            for ag in AgList:
                ...
                for m in Minus:
                    ro = searchRoutingOutlet(m)  
                    createEmptyList(AgSimSeq["AgSimMinus"][ro],
                                    "InsituDivAgents")
                    createEmptyList(Piority["AgSimMinus"][ro],
                                    "InsituDivAgents")
                    AgSimSeq["AgSimMinus"][ro]["InsituDivAgents"].append(ag)
                    Piority["AgSimMinus"][ro]["InsituDivAgents"].append(
                        model["ABM"][agType][member]["Inputs"]["Piority"]
                        )
        ...
        for pm in AgSimSeq:
            for AgTypes in AgSimSeq[pm]:
                for ro in AgSimSeq[pm][AgTypes]:
                    ...
                    AgSimSeq[pm][AgTypes][ro] = list(set(Agents))    

        model["SystemParsedData"]["AgSimSeq"] = AgSimSeq    
        #----------------------------------------
    SummaryDict = {}
    for i in ["SimSeq","RoutingOutlets","ResDamAgents","DamDivAgents",
              "RiverDivAgents","InStreamAgents","AgSimSeq"]:
